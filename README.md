...
This repository is a personal fork/snapshot maintained under
`ducphan1311/9router`.

Upstream source:
- Repository: `https://github.com/decolua/9router`
- Upstream remote in this local checkout: `upstream`
- Synced upstream version: `v0.4.62`
- Synced upstream commit: `ac2fee7`
- Upstream release date shown in git tag history: `2026-05-26`

Changes in this fork compared with the original `decolua/9router v0.4.62`:

- Added Vault-backed run scripts in `package.json`:
  - `npm run dev:vault`
  - `npm run build:vault`
  - `npm run start:vault`
- Added `scripts/vault-env.mjs` to load runtime secrets from HashiCorp Vault before starting Next.js commands.
- Added Gemini 3.1 Flash Lite model entries in:
  - `open-sse/config/providerModels.js`
  - `src/shared/constants/pricing.js`
- Removed hardcoded Google OAuth client IDs/secrets from the pushed snapshot and replaced them with environment variables:
  - `GEMINI_OAUTH_CLIENT_ID`
  - `GEMINI_OAUTH_CLIENT_SECRET`
  - `ANTIGRAVITY_OAUTH_CLIENT_ID`
  - `ANTIGRAVITY_OAUTH_CLIENT_SECRET`

Why the pushed GitHub branch is a clean snapshot:

GitHub Push Protection blocks some historical upstream commits and current source files because they contain Google OAuth credentials. To keep this personal repository pushable, `origin/master` is published as a clean snapshot rather than a full mirror of upstream history. The local checkout still keeps `upstream` linked to the original project so future updates can be fetched from `decolua/9router`.

Local remote layout used for maintenance:

```bash
git remote -v
# origin   git@github.com-personal:ducphan1311/9router.git
...
# re-apply/adjust fork-specific changes if upstream touches the same files
```
